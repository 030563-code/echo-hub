-- The Xero stock feed has written nothing to the ledger since 22 September 2026.
--
-- WHAT STOPPED IT. 20260922140000_xero_items_in_depot_mapping.sql let product_depot_mapping hold a
-- Xero item with no HubSpot SKU (hubspot_sku_code became nullable) and added such rows for every
-- orderable Xero item. This function joins every active mapping row, so from the next snapshot on it
-- tried to write rows with an empty sku into warehouse_stock_levels, whose sku is NOT NULL. The
-- insert failed, the trigger on xero_stock_snapshot failed with it, and PostgREST refused each
-- organisation's whole batch with HTTP 400. The n8n save nodes carry on after an error, so the UK,
-- France and Group runs all still showed success, and the ledger kept the figures of 22 September.
--
-- THE FIX. Only mapping rows that name a SKU feed the finished-goods ledger, which is what this
-- function did before 22 September. A Xero item with no SKU stays in product_depot_mapping for the
-- PO dropdown; it simply has no ledger row to update.
--
-- The body below is the one from 20260922110000_stock_on_hand_floor.sql with one line added.
...
begin
  ...
  mapped as (
    ...
    join public.product_depot_mapping m
     ...
     and m.is_active
     -- A Xero item with no SKU is orderable but has no place in the ledger (see the header).
     and m.hubspot_sku_code is not null
