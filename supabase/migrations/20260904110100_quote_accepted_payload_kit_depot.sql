-- The accepted-quote payload must carry the kit components' real Xero codes.
--
-- notify_quote_accepted() does NOT reuse the xero_item_code that
-- enrich_line_items_with_xero() already stamped on the row; it rebuilds the
-- line array with its own join, and that join was pinned to the deal's depot.
-- A fitting kit on a US-SBD deal splits into HKNA and BUNNA, which are mapped
-- only at Baltimore and Hamilton, so both reached the quote-accepted and
-- quote-accepted-mcs webhooks with a NULL xero_item_code: the Xero quote would
-- be raised with unidentified lines.
--
-- Only the line-array SELECT changes. Everything else, including both webhook
-- posts and the firing condition, is byte-for-byte the function as it stood.
  ...
  IF NEW.deal_status IN ('1170409275')
    ...
    SELECT jsonb_agg(
      jsonb_build_object(
        ...
        -- The depot this LINE dispatches from. Same as the deal's for every
        -- ordinary line; Baltimore for a fitting-kit component, which is where
        -- its Xero item code was resolved and where it actually ships from.
        'depot_code', COALESCE(NULLIF(raw_item->>'ship_from_depot', ''), clean_depot_code)
      )
      -- Stable line order in the payload, rather than the planner's choice.
      ORDER BY ord
    )
    INTO enriched_items
    FROM jsonb_array_elements(NEW.line_items_raw) WITH ORDINALITY AS t(raw_item, ord)
    LEFT JOIN public.product_depot_mapping mapping
      ON mapping.hubspot_sku_code = (raw_item->>'sku')
      AND mapping.depot_code = COALESCE(NULLIF(raw_item->>'ship_from_depot', ''), clean_depot_code);
