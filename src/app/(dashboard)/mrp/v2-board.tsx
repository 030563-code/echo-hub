...
function FlagBadge({ flag }: { flag: string }) {
  return (
    <span className="inline-flex items-center whitespace-nowrap rounded-full border border-[#2a2a2a] bg-[#1e1e1e] px-2 py-0.5 text-[10px] text-[#9ca3af]">
      {humanizeFlag(flag)}
    </span>
  );
}

const TH = "px-4 py-2.5 text-[10px] font-medium uppercase tracking-wider text-[#6b7280]";
...
export default function V2ShadowBoard({ data }: { data: V2BoardData | null }) {
  ...
  if (data === null) {
    body = (
      <div className="rounded-xl border border-[#2a2a2a] bg-[#1a1a2e] px-5 py-4 text-sm text-[#9ca3af]">
    ...
    body = (
      <div className="rounded-xl border border-dashed border-[#2a2a2a] px-6 py-10 text-center">
        <p className="text-sm font-semibold text-[#e5e5e5]">No engine run yet</p>
      ...
      <>
        <div className="overflow-x-auto rounded-xl border border-[#2a2a2a]">
          <table className="w-full text-left">
            ...
            <tbody>
              {data.rows.map((r) => (
                <tr key={r.sku} className="border-b border-[#1e1e1e] last:border-b-0">
                  ...
                  <td className="px-4 py-2.5">
                    <span className="font-mono text-xs font-medium text-[#FF7026]">{r.sku}</span>
                    {r.dlt_days !== null && (
                      <span className="block text-[10px] text-[#4b5563]">DLT {r.dlt_days}d</span>
                    )}
                  </td>
                  <td className="whitespace-nowrap px-4 py-2.5">
                    <span className="text-sm font-bold tabular-nums text-[#e5e5e5]">
                      {formatQty(r.nfp)}
                    </span>
                    {/* Projected NFP: second, muted value ONLY when it differs. */}
                    {projectedDiffers(r.nfp, r.projected_nfp) && (
                      <span
                        className="ml-1.5 text-xs tabular-nums text-[#6b7280]"
                        title="Projected NFP"
                      >
                        → {formatQty(r.projected_nfp)}
                      </span>
                    )}
                    <span className="block text-[10px] tabular-nums text-[#4b5563]">
                      yellow ≤ {formatQty(r.yellow_top)} · green ≤ {formatQty(r.green_top)}
                    </span>
                  </td>
                  <td className="px-4 py-2.5">
                    <span
                      className={`text-sm font-bold tabular-nums ${(r.action_qty ?? 0) > 0 ? "text-[#FF7026]" : "text-[#4b5563]"}`}
                    >
  ...
  return (
    <section className="mb-8">
      <div className="mb-1 flex items-center gap-2">
        ...
        <span className="inline-flex items-center rounded-full border border-[#FF7026]/40 bg-[#FF7026]/10 px-2 py-0.5 text-[10px] font-medium uppercase tracking-wider text-[#FF7026]">
          v2 · shadow
        </span>
      </div>
