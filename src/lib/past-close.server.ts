...
import { byCsoOrder, closeDay, daysPastClose, isPastClose, pastCloseFilters } from '@/lib/past-close'
...
export interface PastCloseSummary {
  /** Every one of the person's open deals past its close date, as HubSpot counts them. */
  total: number
  /** Largest amount first, then the oldest close date, at most ROWS of them. */
  deals: PastCloseDeal[]
}

/** Rows the banner lists. A person with more is told how many more. */
const ROWS = 100

/**
 * The signed-in person's own open deals past their close date, for the Quotes banner.
 *
 ...
 */
export async function myPastCloseDeals(nowMs = Date.now()): Promise<PastCloseSummary | null> {
  ...
  try {
    const response = await hubspotFetch('https://api.hubapi.com/crm/v3/objects/deals/search', {
      method: 'POST',
      body: JSON.stringify({
        filterGroups: [{ filters: pastCloseFilters(ownerId, nowMs) }],
        properties: ['dealname', 'closedate', 'amount', 'deal_currency_code'],
    ...
    return { total: Math.max(data.total ?? 0, deals.length), deals }
  } catch (error) {
